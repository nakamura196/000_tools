...
| `--flat` | off | マニフェストごとのフォルダを作らず、出力先直下に保存する（ファイル名にマニフェスト名が前置されます） |
...
- **Image API 3**: `max`（3.0 で size の値としての `full` は廃止されました。region の `full` は 3.0 でも有効です）
- **Image API 1.x / 2.x**: `full`（`max` は Image API 2.1 で追加されたもので、2.0 以前のサーバは解釈できないため）

サービスが無い（Image API 非対応の）マニフェストでは、リソースの URL をそのまま取得します。この場合 `--size` などは適用できません。

`--size` に指定できる形式はサーバの対応レベルによります。`!1024,1024` や `pct:50` は level 2 相当の機能で、level 0/1 のサーバや、サイズ変更を許可していないサーバは 400/500 を返します（国立国会図書館のデジタルコレクションは `!w,h` を受け付けませんでした）。うまくいかない場合は `1000,`（幅指定）を試すか、既定のまま原寸で取得してください。

同一 Canvas に複数の画像が並ぶ `Choice`（カラー画像と赤外線画像など）は、提供側が既定として示したものを選びます。3.0 では `items` の先頭、2.x（`oa:Choice`）では `default` です。`motivation` が `painting` でない Annotation（本文テキストなど）は無視します。

なお `Choice` の構造そのものは Presentation API 3.0 ではなく W3C Web Annotation Data Model 側の定義で、IIIF Cookbook のレシピ 0033 で例示されています。

### 出力先とファイル名

既定では、マニフェストごとに `{出力先}/{ホスト名}_{識別子}/` を作り、その中に `00001.jpg` のような連番で保存します。連番は Canvas の並び順で、5 桁ゼロ埋めです。painting アノテーションを持たない Canvas は飛ばすため、番号が欠けることがあります（Canvas の位置と番号を一致させるためです）。

- 拡張子は、URL の拡張子 → マニフェストの `format` → `--format` の順に決めます
- `--use-label` を付けると `00001_1丁表.jpg` のようにラベルを併記します（連番は残るので並び順は保たれます）。ファイル名に使えない文字（`/` `:` `*` など）と空白のみ `_` に置き換え、日本語はそのまま残します
- 別々のマニフェストが同じフォルダ名になる場合（`https://ex.org/a/1/manifest.json` と `https://ex.org/b/1/manifest.json` はどちらも `ex.org_1`）、後から来たほうにマニフェスト URI のハッシュ 7 文字を付けて区別します。同じフォルダに混ざって「取得済み」と誤判定されるのを防ぐためです
- `--flat` ではフォルダを分けない代わりに、ファイル名の先頭にマニフェスト名を付けます（`ex.org_1_00001.jpg`）。付けないとマニフェストごとに `00001` から振り直しになり、2 つめ以降が既存ファイル扱いでスキップされます
...
1 枚の失敗は記録して次へ進み、最後にレポートへ集約します（`--fail-fast` で即中断に変更できます）。429 と 5xx に対しては指数バックオフ付きで既定 3 回まで再試行します。マニフェスト自体が取得できない場合も、そのマニフェストだけを失敗として記録し、残りの処理は続きます。失敗が 1 つでもあれば終了コードは 1 になります。
